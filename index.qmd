...
=======
Gibbsampler.Extension <- function(Y, prior.parameters, posterior.parameters, S, N, K) {
  A.prior             <- prior.parameters$A.prior
  V.prior             <- prior.parameters$V.prior
  nu.prior            <- prior.parameters$nu.prior
  S.prior             <- prior.parameters$S.prior
...
>>>>>>> Stashed changes
  ...
  S.bar               <- array(0, dim = c(N, N, S))
  S.bar.inv           <- array(0, dim = c(N, N, S))
  gamma.store         <- numeric(S)
  gamma.store[1]      <- 1
  A.posterior         <- array(0, dim = c(K, N, S))
  B0.tilde            <- array(0, dim = c(N, N, S))
  Bplus.tilde         <- array(0, dim = c(N, K, S))
  Sigma.store         <- array(0, dim = c(N, N, S))
  i.vec = c()
  
  for (i in 2:S) {
    # Calculate S.bar1
    S.bar[,,i]        <- gamma.store[i-1] * diag(N) + t(Y) %*% Y + t(A.prior) %*% diag(1/diag(V.prior)) %*% A.prior - t(A.bar) %*% V.bar.inv %*% A.bar
...
```{r}
posterior.draws     <- Gibbsampler.Extension(Y, prior.parameters, posterior.parameters, S, N, K)
B.draws.restricted  <- ImposeSignRestriction(restrictions,N,p,posterior.draws)
B0.store            <- B.draws.restricted$B0.store
Bplus.store         <- B.draws.restricted$Bplus.store
...
```{r}
#| echo: true
ImposeSignRestriction.empirical <- function (restrictions,N,p,posterior.draws){
...
posterior.parameters <- Post.Dist(X,Y,prior.parameters)
posterior.draws      <- Post.Draw(N,S,p,posterior.parameters)
B.draws.restricted   <- ImposeSignRestriction.empirical(restrictions,N,p,posterior.draws)
