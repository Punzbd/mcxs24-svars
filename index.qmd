...
For $S_{1} = 2,000$ and $S_{2} = 18,000$
...
colnames(B0.inv_df) <- c("C1", "C2", "C3", "C4", "C5", "C6")
rownames(B0.inv_df) <- c( "R1", "R2", "R3", "R4", "R5", "R6")
knitr::kable(B0.inv_df, caption = "Table10: SD of the B0 inverse matrix (extension model)")
```
...
colnames(B0.inv_df) <- c("C1", "C2", "C3", "C4", "C5", "C6")
rownames(B0.inv_df) <- c( "R1", "R2", "R3", "R4", "R5", "R6")
knitr::kable(B0.inv_df, caption = "TableX: Mean of the B0 inverse matrix")
```
...
colnames(B0.inv_df) <- c("C1", "C2", "C3", "C4", "C5", "C6")
rownames(B0.inv_df) <- c( "R1", "R2", "R3", "R4", "R5", "R6")
knitr::kable(B0.inv_df, caption = "TableX: SD of the B0 inverse matrix")
```
...
<div style="text-align: center;">  
##### Figure 18: The trace plot and histogram of B0 inverse (The basic with stochastic volatility model)
</div>
...
```
<div style="text-align: center;">  
##### Figure 19: The impulse response to the monetary shock (extension model) 
</div>
...
<div style="text-align: center;">  
##### Figure 20: The trace plot and histogram of B0 inverse (The basic with stochastic volatility model)
</div>
...
<div style="text-align: center;">  
##### Figure 21: The plot and histogram of gamma (The extension with stochastic volatility model)
</div>
...
```
<div style="text-align: center;">  
##### Figure 22: The posterior mean of the stochastic volatility (The extension with stochastic volatility model)
</div>



Gamma is close to zero, which is different from before incorporating the stochastic volatility which is around XX and the distribution of gamma is 
Sigma square is very high 



### Impulse response function

However, the Figure X: The impulse response to the monetary shock (the extension with stochastic volatility model) displays the point estimate impulse response as a flat line which is the value actually close to zero. The 68% confidence interval is very

Normally we might need more observation to capture the heteroskedasticity which in this case is not enough.


slightly differently from what it represents in Figure 6, the baseline model. The CPI tends to decrease after 1.5 year of the shock and continues declining even after 5 years without a chance to recover. Furthermore, real GDP is observed to decrease from half a year to around 3 years after the shock.
...
```
<div style="text-align: center;">  
##### Figure 23: The trace plot and histogram of B0 inverse (The basic with stochastic volatility model)
</div>
