...
\end{align}
```
$B_+=\big[b_0\;B_1\;\dots\;B_p\big]$ and $x_t=\big[1\;y_{t-1}'\;\dots\;y_{t-p}'\big]$.
...
```{=tex}
\begin{align}
\color{Black}P(A,\Sigma\color{Black}|\gamma) &= \color{Grey}P(A|\Sigma) P(\Sigma\color{Black}|\gamma) \\
\color{Black}A|\Sigma &\sim \color{Grey}\mathcal{MN}_{K \times N} \color{Black}(\underline{A}, \Sigma , \underline{V}) \\
...
Table 8 shows that the mean value of the main diagonal of $B_{+}$ remains positive.
