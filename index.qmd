...
For $S_{1} = 1,000$ and $S_{2} = 11,000$
...
The mean of $\gamma$ is equal `r gamma.mean` and the trace plot and histogram shown in [Figure 6: The plot and histogram of gamma (The extension model)].
...
The trace plot in [Figure 7: The trace plot of A and Sigma (The baseline with stochastic volatility model)] confirms that the value of $A_{21}$ and $A_{32}$ oscillate around 1, while $\Sigma_{11}$ and $\Sigma_{22}$ oscillate around 0.002.
...
The trace plot and histogram in [Figure 8: The trace plot and histogram of B0 matrix (The baseline with stochastic volatility model)] show that the values of $B_{11}$ and $B_{22}$ are widely spread from 0 to around 25, but all of them are positive, consistent with the imposed restriction.
...
The posterior draw mean of $\sigma^{2}$ for each of period display in [Figure 9: The posterior mean of the stochastic volatility]. The $\sigma^{2}$ moves around 2000.
...
The trace plot and histogram depicted in [Figure 10: The trace plot of A and Sigma (The extension with stochastic volatility model)] indicate that the values of $A_{21}$ and $A_{32}$ fluctuate around 1, whereas $\Sigma_{11}$ and $\Sigma_{22}$ are nearly zero.
...
The trace plot and histogram in [Figure 11: The trace plot and histogram of B0 matrix (The extension with stochastic volatility model)] reveal that the values of $B_{11}$ and $B_{22}$ span widely from zero to approximately million, confirming the high mean value reported in Table 19.
...
The mean value of $\gamma$ is equal `r gamma.mean`, which is close to that of the extension model without incorporating stochastic volatility heteroskedasticity. The plot and histogram shown in [Figure 12: The trace plot and histogram of gamma (The extension with stochastic volatility model)].
...
```
<div style="text-align: center;">  
##### Figure 12: The trace plot and histogram of gamma (The extension with stochastic volatility model)
</div>

<br>

The posterior draw mean of $\sigma^{2}$ for each of period is displayed in [Figure 13: The posterior mean of the stochastic volatility (The extension with stochastic volatility model)]. For certain periods, $\sigma^{2}$ exhibits a significant increase to a very large value.
...
And the function for calculating impulse response is as follows.
...
The trace plot and histogram of the restricted variable in [Figure 14: The trace plot and histogram of B0 inverse (The baseline model)] confirms that $B_{0}^{-1}$ shows $B_{11}$ as positive, $B_{21}$ and $B_{31}$ as negative, and $B_{66}$ as positive, consistent with the specified restrictions.
```{r}
...
The trace plot and histogram of the restricted variable in [Figure 16: The trace plot and histogram of B0 inverse (The extension model)] confirms that $B_{0}^{-1}$ shows $B_{11}$ as positive, $B_{21}$ and $B_{31}$ as negative, and $B_{66}$ as positive, consistent with the specified restrictions.
...
<div style="text-align: center;">  
##### Figure 16: The trace plot and histogram of B0 inverse (The extension model)
</div>

<br>

The mean value of $\gamma$ is equal `r gamma.mean`, which is close to zero. The plot and histogram shown in [Figure 17: The trace plot and histogram of gamma (The extension model)] confirms that the draw oscillates around the mean.
...
```
<div style="text-align: center;">  
##### Figure 17: The trace plot and histogram of gamma (The extension model)
</div>
...
The trace plot and histogram of the restricted variable in [Figure 19: The trace plot and histogram of B0 inverse (The basic with stochastic volatility model)] confirms that $B_{0}^{-1}$ shows $B_{11}$ as positive, $B_{21}$ and $B_{31}$ as negative, and $B_{66}$ as positive, consistent with the specified restrictions.
...
The posterior draw mean of $\sigma^{2}$ for each period is displayed in [Figure 20: The posterior mean of the stochastic volatility (The baseline with stochastic volatility model)]. The $\sigma^{2}$ moves around 8000. We can also see that the $\sigma^{2}$ rises up to 60000 in some particular periods.
...
The [Figure 21: The impulse response to the monetary shock (the baseline with stochastic volatility model)] displays the impulse response differently from how it is represented in the previous two models. We do not observe the cash rate normalizing around three years after the shock. Furthermore, the CPI decreases at the same time as the monetary shock. However, it exhibits an increasing trend, eventually recovering five years after the shock.  Additionally, we do not observe the unemployment rate increase that was seen contemporaneously in the previous two models, but it increases slightly after 2 years of the shock. Overall, the impulse response magnitude for all variables in the system of this model is lower compared to the former two models. Hence, considered together with the confidence interval, they are not significantly different from zero.
...
The trace plot and histogram of the restricted variable in [Figure 22: The trace plot and histogram of B0 inverse (The basic with stochastic volatility model)] confirms that $B_{0}^{-1}$ shows $B_{11}$ as positive, $B_{21}$ and $B_{31}$ as negative, and $B_{66}$ as positive, consistent with the specified restrictions.
...
The mean value of $\gamma$ is equal `r gamma.mean`, which is close to zero. This value is also very similar to what it was before the author incorporated stochastic volatility, as shown in [Figure 23: The trace plot and histogram of gamma (The extension with stochastic volatility model)].
...
<div style="text-align: center;">  
##### Figure 23: The trace plot and histogram of gamma (The extension with stochastic volatility model)
</div>

<br>

The posterior draw mean of $\sigma^{2}$ for each period is displayed in [Figure 24: The posterior mean of the stochastic volatility (The extension with stochastic volatility model)]. The $\sigma^{2}$ is significantly higher compared to the previous model.
...
[Figure 25: The impulse response to the monetary shock (the extension with stochastic volatility model)], shows the point estimate of the impulse response as a horizontal line, indicating a value very close to zero. The 68% confidence interval is also flat, suggesting no significant effects on any of the six variables in the system. One possible explanation is that we might need more observations to capture the heteroskedasticity, which, in this case, is not sufficient. This may be due to poor estimation of $\sigma^{2}$ since we have seen the value is incredibly high.
