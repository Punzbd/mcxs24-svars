...
data = data.frame(cashrate, lnrealgdp, inflation, unemprate, lnstockprice, ln_gov_exp)
...
rownames(result_df) <- c("cashrate", "realgdp", "inflation", "unemprate", "stockprice", "govexp")
...
In this section, the autocorrelation and partial autocorrelation analyses are used to detect patterns and check the randomness of time series. [Figure 3: ACF plots] show the autocorrelation remains in all variables, even after 5 years, except _inflation_. The acf of _inflation_ also indicates the significant autocorrelation in first quarter lag, nevertheless, this might occur due to a type I error.
...
for (i in 1:ncol(data)) {
  acf(data[, i], main = c("cashrate", "realgdp", "inflation", "unemprate", "stockprice", "govexp")[i])
}
```
<div style="text-align: center;"> 
##### Figure 3: ACF plots
...
For the pacf in [Figure 4: PACF plots], it is observed that there have not been any significant spikes  for all variables.
```{r PACF plot}
par(mfrow = c(3, 2), oma = c(0, 0, 2, 0))
par(mar=c(4,4,4,2))
...
for (i in 1:ncol(data)) {
  pacf(data[, i], main = c("cashrate", "realgdp", "inflation", "unemprate", "stockprice", "govexp")[i])
}
```
...
### The unit root test
In this section, the Augmented Dickey-Fuller Test (ADF Test) is used to test for the stationarity assumption. The null hypothesis is that the time series data has a unit root (non-stationary). The results are shown in table 2. Only _cashrate_ and _inflation_ can be rejected on the null hypothesis which indicate that these variables are stationary. 
...
rownames(adf_matrix) <- c("cashrate", "realgdp", "inflation", "unemprate", "stockprice", "govexp")
...
```
Then, take the first difference to all variables and rerun the ADF test. The results are shown in table 3. It is reasonable to conclude that all variables except _cashrate_ and _inflation_ is integrated of order 1, while the rest are integrated of order 0, or in other words, they are stationary. 
...
colnames(data_diff1) <- paste0("d", c("cashrate", "realgdp", "inflation", "unemprate", "stockprice", "govexp"))
...
```{=tex}
\begin{align}
P(A,\Sigma|Y,X) &= P(A|\Sigma) P(\Sigma) \\
...
```{r}
plot.ts(Y,main = "1,000 simulations of bi-variate Gaussian random walk", xlab = "Obs.", col = "grey27", lwd = 2, frame.plot = FALSE)
```
