...
#| message: false
#| warning: false
#restriction = diag(c(1,1)) # As we want B0 to have positive value on main diagonal 
...
In Table 8, the values in the first column, which represent the constant term in the equation, are close to zero and exhibit a behavior similar to that of the basic model.
...
And the mean of $\gamma$ is equal `r gamma.mean`
...
```{r prior distribution Emperical}
#| echo: false
#| message: false
#| warning: false
...
```{r Normal-Inverse wishart posterior Empirical}
#| echo: false
#| message: false
#| warning: false
V.bar.inv   = t(X)%*%X + diag(1/diag(V.prior))
...
```{r Sign restriction}
#mon.shock <- c(1,-1,-1,0,0,0)
#fis.shock <- c(0,0,0,0,0,1)

#R1            = diag(mon.shock)
```


```{r Restriction on B0 inverse}
#| echo: false
#| message: false
#| warning: false
...
```{r}
#B0.inv[1,1] >0  & B0.inv[2,1] <0 & B0.inv[3,1] <0

# prod(R1 %*% B0.inv %*% diag(N)[,1] >= 0)
```


After imposing these restrictions on $B_{0}^{-1}$, the mean of $B_{0}^{-1}$ is displayed in Table 9, conforming to the sign restrictions (in the $1^{st} - 3^{rd}$ rows of the  $1^{st}$ column and the $6^{th}$ row of the $6^{th}$ column)
...
knitr::kable(B0.inv_df, caption = "Table9: Mean of the B0 inverse matrix")
```
...
for (s in 1:S){
  ...
  IRF.inf.posterior[,,s]          = J %*% solve(diag(N*p)-A.bold) %*% t(J) %*% B0.inv.store[,,s]
  A.bold.power    = A.bold
  for (i in 1:(h+1)){
    if (i==1){
      IRF.posterior[,,i,s]        = B0.inv.store[,,s]
    } else {
      IRF.posterior[,,i,s]        = J %*% A.bold.power %*% t(J) %*% B0.inv.store[,,s]
...
#| message: false
#| warning: false
IRFs.k1           = apply(IRF.posterior[,1,,],1:2,median)
...
```{r}
S.bar1 <- array(0, dim = c(N, N, S))
S.bar.inv1 <- array(0, dim = c(N, N, S))
#A.posterior_list <- list()
gamma.store <- numeric(S)

for (i in 2:S) {
  S.bar1[,,i] <- gamma.store[i-1] * diag(N) + t(Y) %*% Y + t(A.prior) %*% diag(1/diag(V.prior)) %*% A.prior - t(A.bar) %*% V.bar.inv %*% A.bar
  S.bar.inv1[,,i] <- solve(S.bar1[,,i])
...
```{r Restriction on B0 inverse_Empirical}
#| echo: false
#| message: false
#| warning: false
...
B0.store = array(NA,c(N,N,(S-1)))
Bplus.store = array(NA,c(N,K,(S-1)))
B0.inv.store = array(NA,c(N,N,(S-1)))
...
colnames(B0.inv_df) <- c("C1", "C2", "C3", "C4", "C5", "C6")
rownames(B0.inv_df) <- c( "R1", "R2", "R3", "R4", "R5", "R6")
knitr::kable(B0.inv_df, caption = "Table10: Mean of the B0 inverse matrix (extension model)")
```
...
#| message: false
#| warning: false
IRF.posterior     = array(NA,c(N,N,h+1,(S-1)))
IRF.inf.posterior = array(NA,c(N,N,(S-1)))
FEVD.posterior    = array(NA,c(N,N,h+1,(S-1)))
J                 = cbind(diag(N),matrix(0,N,N*(p-1)))
for (s in 1:(S-1)){
  A.bold          = rbind(t(A.posterior[2:(1+N*p),,s]),cbind(diag(N*(p-1)),matrix(0,N*(p-1),N)))
  IRF.inf.posterior[,,s]          = J %*% solve(diag(N*p)-A.bold) %*% t(J) %*% B0.inv.store[,,s]
...
```
<div style="text-align: center;">  
##### Figure 7: The impulse response to the monetary shock (extension model) 
</div>


<br>

However, the [Figure 7: The impulse response to the monetary shock (extension model)] displays the impulse response slightly differently from what it represents in Figure 6, the baseline model. The CPI tends to decrease after 1.5 year of the shock and continues declining even after 5 years without a chance to recover. Furthermore, real GDP is observed to decrease from half a year to around 3 years after the shock.

As for the unemployment rate, stock price, and government expenditure, there isn't much different from the previous model. However, the cash rate experiences a faster decline than in the basic model, reaching negative values and normalizing after 4 years following the shock. Despite these variations, all of these variables remain not significantly different from zero.
